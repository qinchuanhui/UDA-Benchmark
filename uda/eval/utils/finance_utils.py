...
def to_number(text: str, ignore_scale) -> float:
    ...
    if num is not None:
        if ignore_scale:
            return round(num * negative_flag * percent_flag, 4)
...
def normalize_number(text: str, ignore_scale) -> str:
    if is_number(text):
        return str(to_number(text, ignore_scale))
    else:
        return text


def normalize_finance_answer(text: str, ignore_scale) -> str:
    ...
    parts = [
        white_space_fix(
            remove_articles(normalize_number(remove_punc(lower(token)), ignore_scale))
        )
