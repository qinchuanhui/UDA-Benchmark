...
def get_metrics(
    ...
    f1 = round(f1, 2)
    if LOG_DETAIL:
        print("F1:", f1, "pred: ", predicted_bags, "gt: ", gold_bags)
        ...
        if is_number(ans_str):
            ...
            if ans_num is None:
                ...
                if (
                    ...
                    ans_str = str(ans_num * scale_to_num(scale))
                    # ans_str = str(ans_num) # ignore scale
                else:
...
def add_percent_pred(prediction_strings, pred_scale, pred):
    ...
    if (
        ...
        pred_str = to_number(pred_str, IGNORE_SCALE)
        if pred_str is None:
            return prediction_strings
...
class TaTQAEmAndF1(object):
    ...
    def __call__(self, response: dict):  # type: ignore
        ...
        if prediction.endswith("."):  # remove the last punctuation
    ...
    def get_detail_metric(self):
        ...
        pivot_tab = df.pivot_table(
            index="answer_type", values=["em", "f1"], aggfunc="mean"
        ).fillna(0)
        return pivot_tab
    ...
    def get_raw_pivot_table(self):
        df = pd.DataFrame(self._details)
        pivot_tab = df.pivot_table(
            index="answer_type", values=["em", "f1"], aggfunc="count"
        ).fillna(0)
        return pivot_tab
